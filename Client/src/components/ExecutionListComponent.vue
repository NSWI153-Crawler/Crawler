<template>
  <div class="execution-list" ref="executionList">
    ...
    <div id="dropdown" class="w-64 relative mx-auto my-4">
      ...
      <div id="checkboxes" class="hidden absolute bg-white z-10 border-2 border-dark-bg w-64 overflow-auto rounded shadow max-h-60">
        ...
        <span v-for="execution in uniqueExecutions" :key="execution.id">
          <label class="block cursor-pointer select-none hover:bg-[#1e90ff]" :for="`execution-${execution.id}`">
            <input class="ml-2" type="checkbox" :id="`execution-${execution.id}`" v-model="execution.checked" /> {{ execution.label }}
          </label>
        </span>
      </div>
    </div>
    ...
    <div class="min-w-[500px] h-[500px]">
      <table class="table-auto w-full dark:text-dark-fg">
        ...
        <tr>
          <th class="px-2 button-style" @click="sortTable('label')">Label
          ...
          <th class="px-2 button-style" @click="sortTable('time')">Time
          ...
          <th class="px-2 button-style" @click="sortTable('numberOfSitesCrawled')">Sites Crawled
          ...
          <th class="px-2 button-style" @click="sortTable('status')">Status
        ...
        <tr v-for="execution in paginatedExecutions" :key="execution.id">
          <td class="border border-dark-bg dark:border-dark-fg px-2 py-2">{{ execution.label }}</td>
          <td class="border border-dark-bg dark:border-dark-fg px-2 py-2">{{ formatDate(execution.time) }}</td>
    ...
    <div class="flex justify-center space-x-4 my-4">
      ...
      <span class="py-1 dark:text-dark-fg">Page {{ currentPage }} of {{ Math.ceil(filteredExecutions.length / pageSize) == 0 ? 1 : Math.ceil(filteredExecutions.length / pageSize) }}</span>
      <button
        class="button-style border border-black rounded-lg"
        :class="[((currentPage === Math.ceil(filteredExecutions.length / pageSize)) || filteredExecutions.length == 0) ? 'disabled-state cursor-not-allowed' : 'bg-blue-500 hover:bg-blue-700 text-white font-bold']"
...
const onClickOutside = (event: MouseEvent ) => {
  ...
  if (dropdown && dropdown.contains(event.target as Node))
    return
  if (checkboxes && !checkboxes.contains(event.target as Node))
    hideCheckboxes();
}
